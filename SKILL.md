...
**只允许返回/展示/补价格 status=1 的上架产品。** 睿锋后台查询到的 status≠1（未上架/下架）产品**一律过滤，不展示、不报价、不补价格**——即使它与 OE 完全匹配。

- 后台返回多个产品时：先过滤掉全部 status≠1 的产品，再按 **targetPndSource**（数组）排序：**空数组（直接 OE 匹配）> 含 1 > 含 2（无1）> 仅含 0**，最优匹配排最前。
- 若全部命中产品均未上架：**不得输出这些产品及其价格**，回复应明确告知「后台有匹配产品但均未上架，暂无法报价」，并把该 OE 标记为待补充。
